...
const getRepo = () => {
    ...
    if (!fs_1.default.existsSync('community-fork')) {
        (0, child_process_1.exec)('git clone https://github.com/3speaknetwork/community-fork', (error, stdout) => {
            if (error) {
                console.log(error);
...
const createEnv = () => {
    ...
    let hive_id = prompt(color_1.Color.FgWhite + "Community hive ID <hive-12345>: " + color_1.Color.Reset);
    let theme = prompt(color_1.Color.FgWhite + "Theme <sky|burning|dusk>: " + color_1.Color.Reset);
    let tags = prompt(color_1.Color.FgWhite + "Tags <3spk,spk,OTHERTAG>: " + color_1.Color.Reset);
